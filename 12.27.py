...
def fulltonative (src, a=32, h=6.4):
    ...
    Crop          = core.std.CropRel
    pad           = padding (src, a, a, a, a)
    luma          = ShufflePlanes (pad, 0, vs.GRAY)
    u             = Expr (ShufflePlanes (pad, 1, vs.GRAY), "x 0.5 +")
    v             = Expr (ShufflePlanes (pad, 2, vs.GRAY), "x 0.5 +")
    unew          = Repair (NLMeans (u, d=0, a=a, s=0, h=h, rclip=luma), u, mode=1)
    vnew          = Repair (NLMeans (v, d=0, a=a, s=0, h=h, rclip=luma), v, mode=1)
    uhi           = MakeDiff (unew, gauss (gauss (unew, p=8), p=12))
    vhi           = MakeDiff (vnew, gauss (gauss (vnew, p=8), p=12))
    ufinal        = Expr (MergeDiff (u, uhi), "x 0.5 -")
    vfinal        = Expr (MergeDiff (v, vhi), "x 0.5 -")
    merge         = ShufflePlanes ([luma, ufinal, vfinal], [0, 0, 0], vs.YUV)

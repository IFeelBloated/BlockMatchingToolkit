...
def reconstructor (src, pelclip=None, nrlevel=1, a=4, h1=None, h2=None, divide=4, tr=6, pel=4, dct=5, thsad=2000, thscd1=10000, thscd2=255):
    core          = vs.get_core ()
    NLMeans       = core.knlm.KNLMeansCL
    ...
    difcln        = NLMeans (dif, d=0, a=a, s=1, h=h1, rclip=ref)
    ...
    difcln        = NLMeans (dif, d=0, a=a, s=1, h=h1, rclip=padding (rep, a+1, a+1, a+1, a+1))
    ...
    init          = NLMeans (init, d=0, a=a, s=1, h=h2) if nrlevel == 2 else NLMeans (init, d=0, a=a, s=1, h=hfine)
    def inline (flt, n):
        str       = n * h2 / den + hfine * (1 - n / den)
        flt       = init if n == (divide - 1) else flt
        dif       = MakeDiff (pad, flt)
        dif       = NLMeans (dif, d=0, a=a, s=1, h=str, rclip=flt)
...
def generalnr (src, srclow=None, a=32, h=1.2, sigma=8.0, block_size=8, block_step=1, group_size=32, bm_range=24, bm_step=1, lowpass=8):
    ...
    clip          = MergeDiff (gauss (lowf, p=lowpass), MakeDiff (hif, gauss (hif, p=lowpass)))
    return clip
...
def nrsharp (src):
    ...
    def inline (clp, n):
        ...
        dif       = resample (dif, w*2, h*2, kernel="gauss", a1=100, **fmtc_args)
        clp       = MergeDiff (clp, dif)
        n         = n-1
        return clp if n == 0 else inline (clp, n)
    u2x           = Transpose (NNEDI (Transpose (NNEDI (src, **nnedi_args)), **nnedi_args))
    ublur         = inline (u2x, 4)
    cdif          = resample (MakeDiff (ublur, u2x), w, h, -0.25, -0.25, kernel="gauss", a1=100, **fmtc_args)
    cblur         = MergeDiff (src, cdif)
    mblur         = Median (src)
    blur          = min_dif (cblur, mblur, src)
    clip          = MergeDiff (src, MakeDiff (src, blur))
    return clip
...
def deconvolution (src):
    ...
    hif           = MakeDiff (dcv, gauss (dcv, p=8))
    clip          = MergeDiff (gauss (src, p=8), hif)
    return clip

...
### Denoise ###
def spatialnr (src, overshoot=2.4, ring=2.4, nr=1.2, rpass=1, sstring="0.0:16.0 0.48:8.0 0.64:0.5 1.0:0.0", lowpass=8):
    e       = 2.7182818284590452353602874713527
    core    = vs.get_core ()
    hmax    = max (max (overshoot, ring), nr)
    def ringnr (src, h, n, den):
        flt = core.knlm.KNLMeansCL (src, d=0, a=24, s=1, h=h/den)
        n   = n - 1
        return flt if n == 0 else ringnr (flt, h, n, den)
    pad     = padding (src, 24, 24, 24, 24)
    EC      = core.knlm.KNLMeansCL (pad, d=0, a=24, s=0, h=overshoot*e)
    ...
    NR      = core.knlm.KNLMeansCL (ECLN, d=0, a=24, s=4, h=nr)
    OSC     = core.knlm.KNLMeansCL (NR, d=0, a=24, s=0, h=overshoot)
    RC      = ringnr (OSC, ring, rpass, rpass)
    OSCP    = core.knlm.KNLMeansCL (RC, d=0, a=24, s=0, h=overshoot)
    Dif     = core.std.MakeDiff (pad, OSCP)
    Dif     = core.knlm.KNLMeansCL (Dif, d=0, a=24, s=0, h=hmax, rclip=OSCP)
    NLM     = core.std.MergeDiff (OSCP, Dif)
...
def cmsharp (src, str=1.0):
    ...
    cblur  = cblurf (cblurf (cblurf (up2)))
...
def genlimitclip (src, CM=True):
    core   = vs.get_core ()
    if CM:
       sharp = cmsharp (src, str=3.00)
    else:
       sharp = shrinksharp (src, str=3.00)
    srp    = sharplimit (src, sharp=sharp, str=1.00)
    srpl   = gauss (srp, 16)
    srcl   = gauss (src, 16)
    hif    = core.std.MakeDiff (srp, srpl)
...
def sharpcalmer (soft, dif, limit, peldif, pellimit, vmulti, login, pel=4, tr=6, thsadA=4800, thsadL=400, thscd1=10000, thscd2=255, repmode=13, str=1.00, safelow=False):
    ...
    SRP     = sharplimit (soft, sharp=TL, str=str)
    HiF     = core.std.MakeDiff (SRP, gauss (SRP, 8)) if safelow else 0
    LowF    = gauss (soft, 8) if safelow else 0
    Final   = core.std.MergeDiff (LowF, HiF) if safelow else 0
    return Final if safelow else SRP

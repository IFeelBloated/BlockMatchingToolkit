...
### Denoise ###
def halonr (src, a=32, h=6.4, thr=1.0, elast=1.5):
    core    = vs.get_core ()
    ...
    Final   = limit_dif (pad, MRG, thr=thr, elast=elast)
...
def ringnr (src, a=32, h=2.1333333333333333333333333333333, repeat=1, lowpass=8):
    ...
    def ringnr (src, h, n):
        ...
        n   = n - 1
        return flt if n == 0 else ringnr (flt, h, n)
    Ref1    = ringnr (pad, h, repeat)
    Ref2    = core.knlm.KNLMeansCL (pad, d=0, a=a, s=1, h=h*repeat)
    Ref     = min_dif (Ref1, Ref2, pad)
    Clean   = core.knlm.KNLMeansCL (pad, d=0, a=a, s=1, h=h, rclip=Ref)

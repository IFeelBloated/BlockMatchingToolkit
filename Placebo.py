...
### Denoise ###
def nlmcleansef (src, h1=6.4, h2=2.4, h3=1.2):
    core    = vs.get_core ()
    pad     = padding (src, 24, 24, 24, 24)
    EC      = core.knlm.KNLMeansCL (pad, d=0, a=24, s=0, h=h1, rclip=pad)
    RC      = core.knlm.KNLMeansCL (EC, d=0, a=24, s=1, h=h2, rclip=pad)
    Dif     = core.std.MakeDiff (EC, RC)
    Dif     = core.knlm.KNLMeansCL (Dif, d=0, a=24, s=1, h=h2, rclip=RC)
    RC      = core.std.MergeDiff (RC, Dif)
    Final   = core.knlm.KNLMeansCL (RC, d=0, a=24, s=4, h=h3, rclip=pad)
    clip    = core.std.CropRel (Final, 24, 24, 24, 24)
    return clip

def spatialnrf (src, NLM, DFT, lowpass=16, safe=1):
    core   = vs.get_core ()
    mask   = core.tcanny.TCanny (NLM, op=0, mode=1, gmmax=20)
    EC     = core.std.MakeDiff (NLM, gaussf (NLM, p=1)).std.MergeDiff (gaussf (DFT, p=1))
    HNR    = core.std.MakeDiff (NLM, gaussf (NLM, p=lowpass)).std.MergeDiff (gaussf (DFT, p=lowpass))
    MHNR   = core.std.MaskedMerge (HNR, EC, mask)
    clip   = core.std.MakeDiff (MHNR, gaussf (MHNR, p=safe)).std.MergeDiff (gaussf (src, p=safe))
    return clip

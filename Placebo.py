...
def shrinksharpf (src, str=1.0):
    core   = vs.get_core ()
    blur   = core.ctmf.CTMF (ftoi16 (src), radius=1)
...
def cmsharpf (src, str=1.0):
    ...
    mblur = core.ctmf.CTMF (ftoi16 (src), radius=1)
    mblur = i16tof (mblur)
...
def degrainnf (src, comp, superclip, vmulti, tr=6, pel=4, thsad=400, thscd1=248, thscd2=130):
    ...
    def MDG1 (a):
        ...
        MDG     = core.mv.Degrain1 (ftoi16 (src), super, bv, fv, thsad=thsad, thscd1=thscd1, thscd2=thscd2, plane=0, limit=255)
        return MDG
    ...
    def MDGMerge (start=None, a=2):
        ...
        merge   = core.std.Expr ([start, core.std.SelectEvery (MDGMulti, tr, a)], ["x {a} * {a} 1 + / y {a} 1 + / +".format (a=a)])
        a       = a+1
